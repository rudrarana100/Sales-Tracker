...
const statusBadge = {
  cold: { label: "Cold", class: "bg-slate-800 text-slate-300 border border-slate-700" },
  contacted: { label: "Contacted", class: "bg-blue-950/50 text-blue-400 border border-blue-800" },
  warm: { label: "Warm", class: "bg-amber-950/50 text-amber-400 border border-amber-800" },
  meeting_booked: { label: "Meeting", class: "bg-purple-950/50 text-purple-400 border border-purple-800" },
  proposal_sent: { label: "Proposal", class: "bg-indigo-950/50 text-indigo-400 border border-indigo-800" },
  closed_won: { label: "Won", class: "bg-emerald-950/50 text-emerald-400 border border-emerald-800" },
  closed_lost: { label: "Lost", class: "bg-rose-950/50 text-rose-400 border border-rose-800 line-through" },
};
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/70 dark:border-slate-800 p-5 shadow-xl space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6 pt-1">
          {[
            ...
            <div key={i} className="flex flex-col justify-center rounded-xl border border-slate-800/80 bg-slate-950/40 p-2.5">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 items-stretch">
        ...
        <div className="space-y-6 lg:col-span-2 flex flex-col justify-between">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
            <div className="flex flex-col">
              <SectionCard 
                title={<span className="flex items-center gap-2 text-xs font-bold text-slate-200 uppercase tracking-wider"><Clock className="h-4 w-4 text-blue-400" /> Previous Interaction</span>}
              ...
              >
                <div className="grid grid-cols-2 gap-4 text-xs py-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="font-semibold text-slate-200 capitalize truncate">{currentLead.last_outcome?.replace(/_/g, " ") || "--"}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold uppercase text-slate-400">Last Contact</p>
                    <p className="text-slate-300">{currentLead.last_contact_date ? new Date(currentLead.last_contact_date).toLocaleDateString("en-IN") : "--"}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold uppercase text-slate-400">Next Follow-up</p>
                    <p className="text-slate-300">{currentLead.follow_up_date || "--"}</p>
                  </div>
                </div>
              </SectionCard>
            </div>
            ...
            <div className="flex flex-col">
              <SectionCard 
                title={<span className="flex items-center gap-2 text-xs font-bold text-slate-200 uppercase tracking-wider"><FileText className="h-4 w-4 text-purple-400" /> Recent Notes</span>}
                className="flex-1 flex flex-col justify-between"
              >
                {notes.length === 0 ? (
                  <p className="text-xs text-slate-500 py-6 text-center italic">No notes recorded yet.</p>
                ) : (
                  <div className="space-y-2 flex-1 flex flex-col justify-between">
                    <div className="space-y-2">
                      {visibleNotes.map((note) => (
                        <div key={note.id} className="rounded-xl border border-slate-800 bg-slate-950/40 p-2.5">
          ...
          <SectionCard title={<span className="flex items-center gap-2 text-xs font-bold text-slate-200 uppercase tracking-wider"><History className="h-4 w-4 text-emerald-400" /> Interaction Timeline</span>}>
            {activities.length === 0 ? (
              <p className="text-xs text-slate-500 py-4 text-center italic">No activity history.</p>
            ) : (
                  ...
                  {visibleActivities.map((a) => (
                    <div key={a.id} className="relative flex items-start gap-3 text-xs">
                      ...
                      <div className="space-y-0.5 min-w-0">
                        <p className="font-bold text-slate-200">{a.description}</p>
        ...
        <div className="lg:sticky lg:top-6 flex flex-col h-full">
          <SectionCard 
            title={<span className="flex items-center gap-2 text-xs font-bold text-slate-200 uppercase tracking-wider"><Phone className="h-4 w-4 text-blue-400" /> Log Call Outcome</span>}
          ...
          >
            <div className="space-y-2">
              {[
                ...
                >
                  <span className="flex items-center gap-2.5">
                    <btn.icon className={`h-4 w-4 ${btn.primary ? "text-white" : "text-slate-400"}`} />
                    <span>{btn.label}</span>
                  </span>
              ...
              <button
                onClick={skipLead}
                className="w-full flex items-center justify-center gap-2 rounded-xl border border-dashed border-slate-800 hover:border-slate-700 px-3.5 py-2 text-xs font-semibold text-slate-400 hover:text-slate-200 transition-all mt-3 cursor-pointer"
              >
