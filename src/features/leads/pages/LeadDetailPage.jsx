...
function LeadDetailPage() {
  ...
  async function saveFollowUp() {
    try {
      ...
      const formattedDate = new Date(followUpDate).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" });
      const formattedTime = new Date(`2000-01-01T${followUpTime}`).toLocaleTimeString("en-IN", { hour: "numeric", minute: "2-digit", hour12: true });
      await addActivity({
        lead_id: lead.id,
  ...
  async function saveMeeting() {
    try {
      ...
      await addActivity({ lead_id: lead.id, activity_type: "status_change", description: "Status changed to Meeting Booked" });
      sendMeetingConfirmation(meetLink);
      const formattedDate = new Date(meetingDate).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" });
      const formattedTime = new Date(`2000-01-01T${meetingTime}`).toLocaleTimeString("en-IN", { hour: "numeric", minute: "2-digit", hour12: true });
      await addActivity({ lead_id: lead.id, activity_type: "meeting", description: `Google Meet booked for ${formattedDate} at ${formattedTime}` });
      await fetchLead();
      setTimelineRefresh((prev) => prev + 1);
      setMeetingDate("");
      setMeetingTime("");
  ...
  function sendMeetingConfirmation(meetLink) {
    if (!lead.phone) { alert("No phone number found."); return; }
    let phone = lead.phone.replace(/\D/g, "");
    if (phone.length === 10) phone = "91" + phone;
    const formattedDate = new Date(meetingDate).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" });
    const formattedTime = new Date(`2000-01-01T${meetingTime}`).toLocaleTimeString("en-IN", { hour: "numeric", minute: "2-digit", hour12: true });
  ...
  if (!lead) {
    return <div className="flex h-64 items-center justify-center text-sm text-fog">Loading...</div>;
  }

  return (
    <div className="mx-auto max-w-3xl space-y-4">
      <LeadHeader lead={lead} navigate={navigate} handleLeadUpdate={handleLeadUpdate} />
      ...
      {showMeetingForm && (
        <Card className="border-electric-blue/30 shadow-none">
          <CardHeader className="flex flex-row items-center justify-between border-b border-ash px-4 py-3">
            ...
            <Button size="icon-xs" variant="ghost" onClick={() => setShowMeetingForm(false)}>
          ...
          <CardContent className="space-y-3 p-4">
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" variant="outline" onClick={() => setShowMeetingForm(false)}>
                Cancel
              </Button>
            </div>
          </CardContent>
      ...
      {showFollowUpForm && (
        <Card className="border-ash shadow-none">
          <CardHeader className="flex flex-row items-center justify-between border-b border-ash px-4 py-3">
            ...
            <Button size="icon-xs" variant="ghost" onClick={() => setShowFollowUpForm(false)}>
              <X className="h-3.5 w-3.5" />
            </Button>
          </CardHeader>
          <CardContent className="space-y-3 p-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <NotesCard leadId={lead.id} setTimelineRefresh={setTimelineRefresh} NotesPanel={NotesPanel} />
        <TimelineCard leadId={lead.id} refreshTrigger={timelineRefresh} ActivityTimeline={ActivityTimeline} />
      </div>
    </div>
  );
}
